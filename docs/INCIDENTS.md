...
| 09-14 | the box runner ignores SIGTERM; every restart waits 90 s for SIGKILL | 2 | 09-16: the wait between ticks ends on the signal; the unit gives a tick in flight 120 s (worker/host/main.ts, ledge-tick.service) |
| 09-15 | gateway cached ~900 null headers from the fallback (taken while the primary refused); tick and crawl failed on the same block for 7 h (18:03Z to 01:2xZ), unnoticed | 2 | null answers never cached; null headers retried with the backoff (gateway tests); stall alert to the owner (pipeline/stall_watch.py, ledge-watch.timer, 09-16) |
| 09-18 | the live index evicted launch and graduation rows after 7 days; every token older than a week lost its launch time, age, cohort and time to graduation on its page while the record held them | 2 | rows kept for good, only activity ages out (tick tests); 164,424 launches and 2,913 graduations backfilled from the record; the notice names the record's first day |
