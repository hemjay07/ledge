...
- 2026-09-13 — new definitions, no existing figure changed: **first-buy timing**.
  ...
  Live windows measured today, ~18:00Z (819 `CurveBuy` logs per 1,000 blocks, 35 launches; over 6,000 blocks, 207 launches, 183 with a buy in range): 158 of those 183 first buys were in the launch transaction itself (same `transactionHash` as `TokenLaunched`), and the buyer topic equalled the deployer in only 3 of them — so the launch's own opening buy is routed through something other than the deployer's address in almost every case. Of the 149 launches whose first buy was outside the launch tx, 80 landed within 0–9 blocks (roughly a second) and 26 within 10–19 blocks. "First buy" therefore has two different meanings, kept apart as two record kinds rather than folded into one figure: the **launch-tx buy** (the launch's own opening buy, `inLaunchTx: true`) and the **first outside buy** (the earliest `CurveBuy` on that curve from a different transaction, `inLaunchTx: false`). Sniping is a statement about the second, never the first — a launch-tx buy tells us nothing about who bought it, only that the curve did not open empty.

  For every launch at or after a new state field, `firstBuyIndexedFromBlock` — set once, on whichever run first finds it absent, to that run's own scan start, and never moved again — the crawl now also reads every `CurveBuy` in its scanned windows, by topic0 alone with no address filter (each launch deploys its own curve; there is no single contract to filter to). Launches before that block were never read for buys at all, so a "first" for them would be false; they are outside the population entirely, not a zero. Timing is `ts(buy block) − ts(launch block)` in whole seconds from block headers (never a block-count conversion, per this file's own rule above), and "same block" is a block fact checked before any arithmetic on seconds, its own bucket regardless of what a delta computed from timestamps would otherwise say.

  Published as `number.json`'s new top-level `firstBuy` block, over the whole record (no h24 split): population is every launch at least one hour old at `crawledAt` and at or after `indexedFromBlock`. Three cohort dimensions — `all`, `taxBucket`, `pairClass` — each row carrying `n`, the count and share with a launch-tx buy, and a six-way exclusive breakdown of the outside buy's timing (`sameBlock`, `within1s`, `within3s`, `within5s`, `after5s`, `none`), where `within1s` is delta ≤ 1 s, `within3s` is 1 s < delta ≤ 3 s, `within5s` is 3 s < delta ≤ 5 s, and `after5s` is everything slower. Cumulative shares (`sameBlockShare` through `within5sShare`) fold in every bucket at or before them, exactly as the existing ladder does; `noneShare` is not cumulative. Below n = 30 every share is `null` with `insufficient: true` and the raw counts are still published, the same rule as everywhere else in this file (CONSTRAINTS.md #4).

  **No wallet address is ever written to a first-buy record** (CONSTRAINTS.md #2). The buyer is decoded only long enough to compare it against the launch's own deployer address; the comparison's result, `buyerIsDeployer`, is what is written, never the address itself.

  Raw data: `data/firstbuys/YYYY-MM-DD.jsonl`, partitioned by the buy's own block timestamp like every other partition and deduped on the buy's `(txHash, logIndex)`.
